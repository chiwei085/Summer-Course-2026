...
Response Response::png(std::vector<std::uint8_t> bytes) {
    Response r;
    r.status = 200;
...
void HttpServer::handle_event(mg_connection* c, int ev, void* ev_data) {
    ...
    for (const auto& [key, handler] : impl->routes) {
        ...
        // at embedded NUL bytes -- fatal for binary PNG payloads. Write the
        // status line and headers ourselves, then push the body as a raw
        // byte buffer via mg_send() instead.
        mg_printf(c,
                  "HTTP/1.1 %d %s\r\n"
                  "Content-Type: %s\r\n"
                  "Content-Length: %d\r\n"
