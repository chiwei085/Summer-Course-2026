...
if [[ -n "${WAYLAND_DISPLAY:-}" && -n "${XDG_RUNTIME_DIR:-}" && -S "${XDG_RUNTIME_DIR}/${WAYLAND_DISPLAY}" ]]; then
    exec env UID="$uid" GID="$gid" WAYLAND_DISPLAY="$WAYLAND_DISPLAY" XDG_RUNTIME_DIR="$XDG_RUNTIME_DIR" \
...
if [[ -n "${DISPLAY:-}" ]]; then
    ...
    exec env UID="$uid" GID="$gid" DISPLAY="$DISPLAY" XAUTHORITY="$xauthority" \
